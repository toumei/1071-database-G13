...
CREATE TABLE `apply` (
  ...
  KEY `FK_switchID_apply` (`switchID`),
  CONSTRAINT `FK_employeeID_apply` FOREIGN KEY (`employeeID`) REFERENCES `employee` (`employeeid`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FK_switchID_apply` FOREIGN KEY (`switchID`) REFERENCES `switch` (`switchid`) ON DELETE CASCADE ON UPDATE CASCADE
...
CREATE TABLE `switch` (
  ...
  CONSTRAINT `FK_vendorCode_switch` FOREIGN KEY (`vendorCode`) REFERENCES `vendor` (`vendorcode`) ON DELETE CASCADE ON UPDATE CASCADE
...
-- Dump completed on 2018-12-22  5:54:08
